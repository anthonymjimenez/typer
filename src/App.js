...
function App() {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      let usersObject = {};
      users.data.forEach((user) => (usersObject[user.id] = user));
    ...
    fetch().then((data) => {
      if (user) {
        // here checking if user is already logged in, eventually this would be replaced with cached data (maybe local storage)
        ...
        setUser((userData) => data.usersObject[userData]);
      }
      setPosts(
